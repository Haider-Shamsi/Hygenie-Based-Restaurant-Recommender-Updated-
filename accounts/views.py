...
def _update_hygiene_from_reviews(restaurant):
    ...
    app_reviews = reviews.filter(is_google_review=False)
    google_reviews = reviews.filter(is_google_review=True)

    app_count = app_reviews.count()
    google_count = google_reviews.count()

    # Calculate individual averages
    app_avg = float(app_reviews.aggregate(Avg('rating'))['rating__avg'] or 0.0)
    google_avg = float(google_reviews.aggregate(Avg('rating'))['rating__avg'] or 0.0)

    # 1. Update user_rating on the restaurant model (average of all reviews)
    total = reviews.count()
    if total > 0:
    ...
    # Map non-numeric inspection values to default numeric scores
    rating_str = (restaurant.rating_value or '').strip()
    try:
        inspected_rating = float(rating_str)
    except ValueError:
        # Fallback mappings for standard text values in dataset
        mapping = {
            'Pass': 85.0,
            'Exempt': 80.0,
            'Awaiting Inspection': 70.0,
            'Improvement Required': 50.0,
        }
        inspected_rating = mapping.get(rating_str, 70.0)

    # 2. Calculate the weighted hygiene score.
    # Overall score = Inspected Rating (70%) + User Reviews (30%)
    # Within the 30% Reviews portion, we weight Google reviews at 25% (20-30% range) and local reviews at 75%.
    prev_score = restaurant.hygiene_score

    if app_count > 0 and google_count > 0:
        # Both exist: Google reviews affect 7.5% of the total score, local reviews affect 22.5%
        new_score = 0.7 * inspected_rating + 0.225 * (app_avg * 20.0) + 0.075 * (google_avg * 20.0)
    elif app_count > 0:
        # Only local reviews exist: they affect the full 30%
        new_score = 0.7 * inspected_rating + 0.3 * (app_avg * 20.0)
    elif google_count > 0:
        # Only Google reviews exist: they affect 7.5% of total score, and the remaining 22.5% is reallocated to inspected rating
        new_score = 0.925 * inspected_rating + 0.075 * (google_avg * 20.0)
    else:
        # No reviews exist
        new_score = inspected_rating
...
class OwnerReviewsView(APIView):
    ...
    def get(self, request):
        ...
        for review in reviews:
            ...
            results.append({
                'id': str(review.id),
                'user_name': review.google_reviewer_name if review.is_google_review else (review.user.username if review.user else 'Anonymous'),
...
class OwnerSyncGoogleReviewsView(APIView):
    ...
    def _import_demo_reviews(self, restaurant, google_error_reason):
        import hashlib
        import datetime
        ...
        # Predefined demo reviews representing Google places reviews
        demo_data = [
            {
                'author_name': 'Alex Johnson (Google)',
                'rating': 5,
                'text': f"Absolutely loved the dining experience at {restaurant.business_name}! The food was fresh, well-prepared, and the kitchen looked exceptionally clean. Standard hygiene is top-notch.",
                'time_offset': 1
            },
            {
                'author_name': 'Sarah Miller (Google)',
                'rating': 2,
                'text': f"Food at {restaurant.business_name} tasted okay, but I was extremely disappointed by the cleanliness. The tables were sticky and there was a fly buzzing around the food counter.",
                'time_offset': 3
            },
            {
                'author_name': 'David Smith (Google)',
                'rating': 4,
                'text': f"Decent service and friendly staff. Hygiene looked fine and overall clean environment. Will visit again.",
                'time_offset': 5
            }
        ]
        ...
        for item in demo_data:
            ...
            hash_base = f"{rating}:{text}:{author_name}"
            r_hash = hashlib.md5(hash_base.encode('utf-8')).hexdigest()
            
            if RestaurantReview.objects.filter(google_review_hash=r_hash).exists():
                duplicate_count += 1
                continue
                
            review_date = now - datetime.timedelta(days=item['time_offset'])
            
            # Analyze sentiment using the local TF-IDF model
            nlp_result = _analyze_review_nlp(text)
            ...
            review = RestaurantReview(
                ...
                google_reviewer_name=author_name,
                google_review_hash=r_hash,
            ...
            try:
                ...
                details_list.append({
                    'author': author_name,
                    'rating': rating,
                    'comment': text[:100] + ('...' if len(text) > 100 else ''),
        ...
        return Response({
            'success': True,
            'message': f"[DEMO MODE] Google API billing is disabled. Synced 3 mock Google reviews to demonstrate the sentiment analysis and hygiene score updates.",
            'reviews_imported': imported_count,
            'reviews_details': details_list,
            'credits_saved': False,
            'hygiene_score': restaurant.hygiene_score,
            'demo_mode': True
        }, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        ...
        # Check cooldown limit (24 hours) to prevent credit exhaustion
        if restaurant.last_google_sync:
            time_diff = now - restaurant.last_google_sync
            if time_diff < datetime.timedelta(hours=24):
                hours_left = 24 - (time_diff.total_seconds() / 3600.0)
                return Response({
                    'success': True,
                    'message': f"Google reviews synced recently ({(time_diff.total_seconds() / 3600.0):.1f} hours ago). Google Places API calls were skipped to conserve your free credits.",
                    'reviews_imported': 0,
                    'credits_saved': True,
                    'hygiene_score': restaurant.hygiene_score,
                    'cooldown_hours_left': hours_left
                }, status=status.HTTP_200_OK)

        api_key = getattr(settings, 'GOOGLE_PLACES_API_KEY', None)
        if not api_key:
            # Fall back to checking environment variable directly
            api_key = os.environ.get('GOOGLE_PLACES_API_KEY', None)
            
        if not api_key:
            return self._import_demo_reviews(restaurant, "Google Places API key is not configured.")

        # 1. Resolve place_id if not present
        place_id = restaurant.google_place_id
        if not place_id:
            # Clean and construct the text query for find place API
            query_str = f"{restaurant.business_name}, {restaurant.address}, {restaurant.province}"
            ...
            try:
                ...
                if resp.status_code != 200:
                    return self._import_demo_reviews(restaurant, f"Google Search API returned error status {resp.status_code}.")
                data = resp.json()
                if data.get('status') == 'OK' and data.get('candidates'):
                    ...
                    return self._import_demo_reviews(restaurant, data.get('error_message', 'REQUEST_DENIED'))
                else:
                    return Response({
                        'detail': f"Could not find restaurant '{restaurant.business_name}' on Google Places. Google status: {data.get('status', 'Unknown')}."
                    }, status=status.HTTP_400_BAD_REQUEST)
            except Exception as e:
                return self._import_demo_reviews(restaurant, f"Find Place error: {str(e)}")
        ...
        try:
            ...
            if resp.status_code != 200:
                return self._import_demo_reviews(restaurant, f"Google Details API returned error status {resp.status_code}.")
            data = resp.json()
            if data.get('status') == 'REQUEST_DENIED':
                return self._import_demo_reviews(restaurant, data.get('error_message', 'REQUEST_DENIED'))
            if data.get('status') != 'OK':
                return Response({
                    'detail': f"Google Details API returned status: {data.get('status')}."
                }, status=status.HTTP_400_BAD_REQUEST)
            
            result = data.get('result', {})
            google_reviews = result.get('reviews', [])
        except Exception as e:
            return self._import_demo_reviews(restaurant, f"Place Details error: {str(e)}")
        ...
        for gr in google_reviews:
            author_name = gr.get('author_name', 'Google Reviewer')
            rating = int(gr.get('rating', 5))
            text = gr.get('text', '').strip()
            epoch_time = gr.get('time')

            # We generate a unique hash based on text, rating, and author name
            hash_base = f"{rating}:{text}:{author_name}"
            r_hash = hashlib.md5(hash_base.encode('utf-8')).hexdigest()
            ...
            # Compute review date
            if epoch_time:
                review_date = timezone.make_aware(datetime.datetime.fromtimestamp(epoch_time))
            else:
                review_date = now

            # Clean and run sentiment analysis using existing project logic
            nlp_result = _analyze_review_nlp(text)
        ...
        # If any reviews were imported, we run the existing hygiene calculation logic
        if imported_count > 0:
            _update_hygiene_from_reviews(restaurant)
            restaurant.refresh_from_db()

        # Update last sync time to implement throttling and save future credits
        restaurant.last_google_sync = now
        restaurant.save(update_fields=['last_google_sync'])

        return Response({
            'success': True,
            'message': f"Synced Google reviews. Imported {imported_count} new review(s), skipped {duplicate_count} existing review(s).",
            'reviews_imported': imported_count,
            'reviews_details': details_list,
            'credits_saved': False,
            'hygiene_score': restaurant.hygiene_score
